...
def run_train(wandb_project_name, wandb_run_name):
    # hyperparameters
    batch_size = 2
...
def main():
    parser = argparse.ArgumentParser(
        description = " Train on BraTS dataset"
    )

    parser.add_argument(
        "--wandb-project-name",
        type= str,
        default="brats21-baseline-unetr",
        help="The project name to be used in the wanbd for logging"
    )

    parser.add_argument(
        "--wandb-run-name",
        type= str,
        help="Name of the run to be placed in wandb"
    )

    args = parser.parse_args()

    run_train(args.wandb_project_name, wandb_run_name = args.wandb_run_name)
